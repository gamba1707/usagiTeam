    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float2 node_990 = (float2((o.uv0.r*_WaveUScaleforUV_var),o.uv0.g)/_UVScale_var);
                float2 node_1015 = (node_990+node_9290*float2(0.5,0));
                float3 node_119 = UnpackNormal(tex2Dlod(_WaveNormalMap,float4(TRANSFORM_TEX(node_1015, _WaveNormalMap),0.0,0)));
                float2 node_6703 = ((node_990*0.7)+node_9290*float2(-0.2,2));
                float3 _node_1007_copy = UnpackNormal(tex2Dlod(_WaveNormalMap,float4(TRANSFORM_TEX(node_6703, _WaveNormalMap),0.0,0)));
                ...
                float2 node_1159 = o.uv0.rg;
                float2 node_1851 = float2((node_1159.r*0.5),(node_1159.g*2.0));
                float4 _WhiteWavesNormalMap_var = tex2Dlod(_WhiteWavesNormalMap,float4(TRANSFORM_TEX(node_1851, _WhiteWavesNormalMap),0.0,0));
                ...
                float2 node_9901 = (float2(((o.uv0.r+_MaskUOffsetforUV_var)*_MaskUScaleforUV_var),o.uv0.g)+(node_6870*_WhiteWavesnoiseIntensity_var));
                float2 node_9958 = ((node_9901*0.9)+0.01);
                float4 _MaskforWhiteWave_var = tex2Dlod(_MaskforWhiteWave,float4(TRANSFORM_TEX(node_9958, _MaskforWhiteWave),0.0,0));
                float3 node_3741 = lerp(((node_6859*_NormalMapIntensity_var)+(0.5-(_NormalMapIntensity_var/2.0))),_WhiteWavesNormalMap_var.rgb,_MaskforWhiteWave_var.r);
                v.vertex.xyz += (node_3741*0.1);
